...
void ChunkShape::make_convex_from_siblings() {
	Node *p = get_parent();
	if (!p) {
		return;
	}
...
void ChunkShape::_update_in_shape_owner(bool p_xform_only) {
...
void ChunkShape::_notification(int p_what) {
	switch (p_what) {
...
void ChunkShape::resource_changed(Ref<Resource> res) {
}
#endif

PackedStringArray ChunkShape::get_configuration_warnings() const {
	PackedStringArray warnings = Node3D::get_configuration_warnings();

	CollisionObject3D *col_object = Object::cast_to<CollisionObject3D>(get_parent());
	if (col_object == nullptr) {
		warnings.push_back(RTR("ChunkShape only serves to provide a collision shape to a CollisionObject3D derived node.\nPlease only use it as a child of Area3D, StaticBody3D, RigidBody3D, CharacterBody3D, etc. to give them a shape."));
	}

	if (!shape.is_valid()) {
		warnings.push_back(RTR("A shape must be provided for ChunkShape to function. Please create a shape resource for it."));
	}
	...
	if (!(Math::is_zero_approx(scale.x - scale.y) && Math::is_zero_approx(scale.y - scale.z))) {
		warnings.push_back(RTR("A non-uniformly scaled ChunkShape node will probably not function as expected.\nPlease make its scale uniform (i.e. the same on all axes), and change the size of its shape resource instead."));
	}

	return warnings;
}

void ChunkShape::_bind_methods() {
#ifndef DISABLE_DEPRECATED
	ClassDB::bind_method(D_METHOD("resource_changed", "resource"), &ChunkShape::resource_changed);
#endif
	ClassDB::bind_method(D_METHOD("set_shape", "shape"), &ChunkShape::set_shape);
	ClassDB::bind_method(D_METHOD("get_shape"), &ChunkShape::get_shape);
	ClassDB::bind_method(D_METHOD("set_disabled", "enable"), &ChunkShape::set_disabled);
	ClassDB::bind_method(D_METHOD("is_disabled"), &ChunkShape::is_disabled);
	ClassDB::bind_method(D_METHOD("make_convex_from_siblings"), &ChunkShape::make_convex_from_siblings);
	ClassDB::set_method_flags("ChunkShape", "make_convex_from_siblings", METHOD_FLAGS_DEFAULT | METHOD_FLAG_EDITOR);

	ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "shape", PROPERTY_HINT_RESOURCE_TYPE, "Shape3D"), "set_shape", "get_shape");
	ADD_PROPERTY(PropertyInfo(Variant::BOOL, "disabled"), "set_disabled", "is_disabled");
}

void ChunkShape::set_shape(const Ref<Shape3D> &p_shape) {
	if (p_shape == shape) {
		return;
	}
	if (shape.is_valid()) {
...
Ref<Shape3D> ChunkShape::get_shape() const {
	return shape;
}

void ChunkShape::set_disabled(bool p_disabled) {
	disabled = p_disabled;
	update_gizmos();
	if (collision_object) {
...
bool ChunkShape::is_disabled() const {
	return disabled;
}
